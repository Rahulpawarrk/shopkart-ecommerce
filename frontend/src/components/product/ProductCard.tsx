...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="group bg-white rounded-3xl border border-slate-200/90 overflow-hidden hover:shadow-card hover:border-blue-300 transition duration-300 flex flex-col relative justify-between">
      ...
      <div className="p-2.5 sm:p-4 lg:p-5 flex flex-col flex-1 justify-between space-y-2 sm:space-y-3.5">
        <div>
          {product.brand && (
            <p className="text-[9px] sm:text-[10px] font-bold tracking-wider text-blue-600 uppercase mb-0.5 sm:mb-1 truncate">
              {product.brand}
            </p>
          )}
          ...
          <Link
            to={`/products/${product.slug || product.productId}`}
            className="font-bold text-xs sm:text-sm text-slate-900 hover:text-blue-600 line-clamp-2 leading-tight sm:leading-snug group-hover:underline"
          >
            {product.productName}
          </Link>
          ...
          <div className="mt-1.5 sm:mt-2">
            <RatingStars rating={product.ratingAverage || 0} count={product.ratingCount || 0} size="sm" />
          </div>

          {/* Price */}
          <div className="mt-1.5 sm:mt-2.5 flex items-baseline gap-1.5 sm:gap-2 flex-wrap">
            <span className="font-black text-sm sm:text-base lg:text-lg text-slate-900">
          ...
          {product.inStock && product.stockQuantity <= 5 && (
            <p className="text-[9px] sm:text-[10px] font-bold text-amber-600 mt-0.5 sm:mt-1">
        ...
        <div className="pt-2 sm:pt-3 border-t border-slate-100 flex items-center gap-1.5 sm:gap-2">
          <button
            ...
            className={`flex-1 py-1.5 sm:py-2.5 px-1 sm:px-2 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold transition flex items-center justify-center gap-1 shadow-xs ${
              product.inStock
          ...
          <button
            ...
            disabled={!product.inStock}
            className={`flex-1 py-1.5 sm:py-2.5 px-1 sm:px-2 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold transition flex items-center justify-center gap-1 shadow-sm text-white ${
              product.inStock
                ? 'bg-[#2874F0] hover:bg-blue-700 active:scale-95 cursor-pointer'
