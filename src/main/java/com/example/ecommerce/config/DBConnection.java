 ...
 * Manages database lifecycle, connection pooling, and configuration
 * externalization.
 */
public final class DBConnection {
     ...
     * The caller is responsible for closing this connection (e.g. using
     * try-with-resources).
     *
    ...
    private static void initDataSource() {
        try {
            ...
            boolean isPostgres = (jdbcUrl != null && jdbcUrl.startsWith("jdbc:postgresql:"))
                    || "org.postgresql.Driver".equals(driver);
            if (isPostgres) {
            ...
            // Auto-migrate schema updates if missing (e.g. password_reset_tokens, phone
            // column)
            autoMigrateSchema();
        } catch (Exception e) {
    ...
    private static void autoMigrateSchema() {
        String[] ddlStatements = new String[] {
                ...
                // 14. Drop legacy restrictive check constraints on order_status and
                // payment_method
    ...
    private static Properties loadProperties() {
        ...
        try (InputStream in = DBConnection.class.getClassLoader().getResourceAsStream("db.properties")) {
            if (in != null) {
                ...
                logger.info(
                        "db.properties not found in classpath; relying on environment variables (DB_URL / DATABASE_URL).");
            }
        } catch (Exception e) {
