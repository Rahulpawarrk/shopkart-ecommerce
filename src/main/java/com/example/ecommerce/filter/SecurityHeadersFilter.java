...
public class SecurityHeadersFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (response instanceof HttpServletResponse) {
            ...
            // 5. Content Security Policy (CSP) — Allowing required application inline handlers & integrations (unsafe-eval disabled)
            httpResponse.setHeader("Content-Security-Policy",
                    "default-src 'self'; " +
                    "script-src 'self' 'unsafe-inline' https://checkout.razorpay.com https://*.razorpay.com https://cdn.jsdelivr.net; " +
