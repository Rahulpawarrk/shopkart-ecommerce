        ...
        <div class="admin-layout">
            ...
            <section>
                <div style="background: #ffffff; padding: 1.25rem 1.5rem; border-radius: var(--radius-md); border: 1px solid #e2e8f0; margin-bottom: 1.25rem;">
                    <h1 style="font-size: 1.5rem; font-weight: 800; color: #0f172a; margin-bottom: 0.25rem;">Customer Order Fulfillment</h1>
                    <p style="color: #64748b; font-size: 0.875rem;">Track shipments, dispatch orders, verify invoice breakdowns, and process logistics.</p>
                </div>
                ...
                <div class="grid grid-cols-4" style="margin-bottom: 1.25rem;">
                    ...
                    <div class="stat-card" style="border-top: 4px solid #f59e0b;">
                        ...
                        <div class="stat-value" style="color: #f59e0b;">${stats.processingOrders + stats.pendingOrders}</div>
                    </div>
                    <div class="stat-card" style="border-top: 4px solid #3b82f6;">
                        <div class="stat-label">In Transit (Shipped)</div>
                        <div class="stat-value" style="color: #3b82f6;">${stats.inTransitOrders}</div>
                    </div>
                    <div class="stat-card" style="border-top: 4px solid #10b981;">
                        <div class="stat-label">Delivered Orders</div>
                        <div class="stat-value" style="color: #10b981;">${stats.deliveredOrders}</div>
                    </div>
                </div>

                <!-- Filter Tabs -->
                <div class="tabs" style="flex-wrap: wrap;">
                    <a href="${pageContext.request.contextPath}/admin/orders" class="tab-item ${empty status || status == 'ALL' ? 'active' : ''}">All Orders (${stats.totalOrders})</a>
                    <a href="${pageContext.request.contextPath}/admin/orders?status=PROCESSING" class="tab-item ${status == 'PROCESSING' ? 'active' : ''}">📦 Processing (${stats.processingOrders})</a>
                    <a href="${pageContext.request.contextPath}/admin/orders?status=DISPATCHED" class="tab-item ${status == 'DISPATCHED' ? 'active' : ''}">🚚 Dispatched (${stats.dispatchedOrders})</a>
                    <a href="${pageContext.request.contextPath}/admin/orders?status=IN_TRANSIT" class="tab-item ${status == 'IN_TRANSIT' || status == 'SHIPPED' ? 'active' : ''}">✈️ In Transit (${stats.inTransitOrders})</a>
                    <a href="${pageContext.request.contextPath}/admin/orders?status=OUT_FOR_DELIVERY" class="tab-item ${status == 'OUT_FOR_DELIVERY' ? 'active' : ''}">🛵 Out for Delivery (${stats.outForDeliveryOrders})</a>
                    <a href="${pageContext.request.contextPath}/admin/orders?status=DELIVERED" class="tab-item ${status == 'DELIVERED' ? 'active' : ''}">✅ Delivered (${stats.deliveredOrders})</a>
                    <a href="${pageContext.request.contextPath}/admin/orders?status=CANCELLED" class="tab-item ${status == 'CANCELLED' ? 'active' : ''}">✕ Cancelled (${stats.cancelledOrders})</a>
                </div>

                <!-- Search Form -->
                <div class="card" style="padding: 1rem 1.25rem; margin-bottom: 1.25rem;">
                    <form action="${pageContext.request.contextPath}/admin/orders" method="GET" style="display: flex; gap: 1rem;">
                        <input type="hidden" name="status" value="${status}">
                        <input type="text" name="q" placeholder="Search by Order #, Customer Full Name, Email..." value="<c:out value='${keyword}' />" 
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <c:forEach var="order" items="${pagination.items}">
                                            <tr>
                                                <td><code style="background: #f1f5f9; padding: 0.2rem 0.4rem; border-radius: 4px; color: #475569; font-weight: 700;"><c:out value="${order.orderNumber}" /></code></td>
                                                <td>
                                                    <strong style="color: #0f172a;"><c:out value="${order.customerName}" /></strong><br>
                                                    <small style="color: #64748b;"><c:out value="${order.customerEmail}" /></small>
                                                </td>
                                                <td><small style="color: #64748b;">${order.createdAt}</small></td>
                                                <td>
                                                    <span style="font-weight: 700; color: #334155;"><c:out value="${order.paymentMethod}" /></span>
                                                    <c:choose>
                                                        <c:when test="${order.paymentMethod == 'COD'}">
                                                            <c:if test="${order.paymentStatus == 'PAID' || order.orderStatus == 'DELIVERED'}">
                                                                <br><small class="badge badge-success" style="font-size: 0.65rem;">PAID</small>
                                                            </c:if>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <br><small class="badge ${order.paymentStatus == 'PAID' ? 'badge-success' : 'badge-info'}" style="font-size: 0.65rem;"><c:out value="${order.paymentStatus}" /></small>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </td>
                                                <td><strong style="color: #0f172a; font-size: 0.95rem;">₹<fmt:formatNumber value="${order.totalAmount}" minFractionDigits="0" /></strong></td>
                                                <td>
                                                    <c:choose>
                                                        ...
                                                        <c:otherwise>
                                                            <span class="badge badge-warning">⏳ <c:out value="${order.orderStatus}" /></span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </td>
