...
<head>
    ...
    <title>Security & Data Audit Ledger | ShopKart Console</title>
        ...
        <div class="admin-layout">
            ...
            <section>
                <div style="background: #ffffff; padding: 1.25rem 1.5rem; border-radius: var(--radius-md); border: 1px solid #e2e8f0; margin-bottom: 1.25rem;">
                    <h1 style="font-size: 1.5rem; font-weight: 800; color: #0f172a; margin-bottom: 0.25rem;">Security & Data Audit Ledger</h1>
                    <p style="color: #64748b; font-size: 0.875rem;">
                        Immutable audit trail of administrative modifications, inventory restocks, price changes, and state transitions.
                    </p>
                </div>

                <!-- Filter Box -->
                <div class="card" style="padding: 1rem 1.25rem; margin-bottom: 1.25rem;">
                    <form action="${pageContext.request.contextPath}/admin/audit-logs" method="GET" style="display: flex; gap: 1rem;">
                        <input type="text" name="q" placeholder="Search by Actor user email, action type, or payload string..." value="<c:out value='${keyword}' />" 
                               class="form-input" style="flex: 1;">
                        <button type="submit" class="btn btn-primary">Filter Logs</button>
                        <a href="${pageContext.request.contextPath}/admin/audit-logs" class="btn btn-secondary">Reset</a>
                    </form>
                </div>
                        ...
                        <table class="admin-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: right;">Updated Value</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:choose>
                                    <c:when test="${empty pagination.items}">
                                        <tr><td colspan="6" style="text-align: center; padding: 2.5rem; color: #94a3b8;">No audit events recorded matching criteria.</td></tr>
                                    </c:when>
                                    <c:otherwise>
                                        <c:forEach var="log" items="${pagination.items}">
                                            <tr>
                                                <td><small style="color: #64748b;">${log.createdAt}</small></td>
                                                <td>
                                                    <strong style="color: #0f172a;"><c:out value="${log.userEmail != null ? log.userEmail : 'SYSTEM / AUTOMATION'}" /></strong><br>
                                                    <small style="color: #94a3b8;">IP: <c:out value="${log.ipAddress != null ? log.ipAddress : '127.0.0.1'}" /></small>
                                                </td>
                                                <td><span class="badge badge-info"><c:out value="${log.action}" /></span></td>
                                                <td>
                                                    <strong style="color: #334155;"><c:out value="${log.entityName}" /></strong>
                                                    <c:if test="${not empty log.entityId}">
                                                        <code style="background: #f1f5f9; padding: 0.15rem 0.35rem; border-radius: 4px; color: #64748b;">#${log.entityId}</code>
                                                    </c:if>
                                                </td>
                                                <td><small style="color: #94a3b8;"><c:out value="${log.oldValue}" /></small></td>
                                                <td style="text-align: right;"><small style="color: #10b981; font-weight: 700;"><c:out value="${log.newValue}" /></small></td>
                                            </tr>
                                        </c:forEach>
                                    </c:otherwise>
                                </c:choose>
